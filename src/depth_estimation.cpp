...
void DepthEstimation::InitializePointcloudWorkers() {
  ...
  RCLCPP_WARN(
      this->get_logger(),
      "OpenMP not available. Pointcloud computation will be single-threaded.");
#endif

  shutdown_requested_ = false;

  for (int i = 0; i < num_pointcloud_workers_; ++i) {
    pointcloud_workers_.emplace_back([this]() { PointcloudWorkerLoop(); });
  }
...
void DepthEstimation::EnqueueDisparity(DisparityPayload &&payload) {
  ...
  while (pointcloud_queue_.size() >= MAX_QUEUE_SIZE) {
    ...
    RCLCPP_WARN_THROTTLE(this->get_logger(), *this->get_clock(), 1000,
                         "Dropping pointcloud frame %lu - queue full",
                         dropped.frame_id);
...
void DepthEstimation::ProcessPointcloudAsync(DisparityPayload &payload) {
  ...
  Eigen::Matrix4f combined_transform =
      transform_cam0_to_centroid_ * payload.transform_rect_left_to_cam0;

  // For combined mode: write directly to PointCloud2 buffer (skip PCL
  // intermediate) For individual mode: use PCL cloud
  ...
  if (publish_combined_pointcloud_) {
    ...
    std::shared_ptr<FrameCloudAggregator> aggregator;
    {
      ...
      if (it == frame_aggregators_.end()) {
        ...
        size_t points_per_pair =
            payload.resolution.width * payload.resolution.height;
        auto new_aggregator = std::make_shared<FrameCloudAggregator>(
            stereo_pairs_.size(), points_per_pair, pointcloud_frame_id_);
    ...
    DisparityToPointCloud2Direct(
        payload.disparity_map, payload.K_rect_left, payload.baseline_meters,
        payload.baseline_scale, payload.offset_factor, combined_transform, dst);

    cloud_time_ms =
        std::chrono::duration<double, std::milli>(
            std::chrono::high_resolution_clock::now() - t_start_cloud)
            .count();
    ...
    if (count == static_cast<int>(stereo_pairs_.size())) {
      ...
      auto msg_to_publish = std::make_unique<sensor_msgs::msg::PointCloud2>(
          std::move(aggregator->combined_msg));
    ...
    transformed_cloud = DisparityToPointCloud(
        payload.disparity_map, payload.K_rect_left, payload.baseline_meters,
        payload.baseline_scale, payload.offset_factor, combined_transform);
  ...
  if (payload.debug_grid) {
    ...
    cv::Mat valid_mask =
        (payload.disparity_map > 0.0f) & (payload.disparity_map <= 128.0f);
    cv::Mat disparity_normalized;
    cv::normalize(payload.disparity_map, disparity_normalized, 0, 255,
                  cv::NORM_MINMAX, CV_8U, valid_mask);
    ...
    cv::putText(display_image, label + " (Disp)",
                cv::Point(left_vis.cols + 10, 30), cv::FONT_HERSHEY_SIMPLEX,
                0.7, cv::Scalar(255, 255, 255), 2);

    viz_time_ms = std::chrono::duration<double, std::milli>(
                      std::chrono::high_resolution_clock::now() - t_start_viz)
                      .count();
...
void DepthEstimation::PrintFrameTimingReport(uint64_t frame_id) {
  ...
  for (const auto &pair_name : stereo_pairs_) {
    double rect_ms = timing_data->rect_times.count(pair_name)
                         ? timing_data->rect_times[pair_name]
                         : 0.0;
    double infer_ms = timing_data->infer_times.count(pair_name)
                          ? timing_data->infer_times[pair_name]
                          : 0.0;
    double cloud_ms = timing_data->cloud_times.count(pair_name)
                          ? timing_data->cloud_times[pair_name]
                          : 0.0;
    double viz_ms = timing_data->viz_times.count(pair_name)
                        ? timing_data->viz_times[pair_name]
                        : 0.0;

    report << "  [" << pair_name << "] Rect:" << rect_ms
           << " | Infer:" << infer_ms << " | Cloud:" << cloud_ms;
    report << " | Viz:" << viz_ms;
    report << " ms\n";
  ...
  if (publish_combined_pointcloud_) {
    report << " | Combined Publish: " << timing_data->combined_publish_ms
           << " ms";
  }
  ...
  double total_wall_ms = std::chrono::duration<double, std::milli>(
                             t_now - timing_data->callback_start)
                             .count();
...
void DepthEstimation::LogFrameTiming(uint64_t frame_id) {
  ...
  double timestamp =
      timing_data->header.stamp.sec + timing_data->header.stamp.nanosec * 1e-9;
  ...
  for (const auto &pair_name : stereo_pairs_) {
    ...
    timing_file_ << "," << rect_ms << "," << infer_ms << "," << cloud_ms << ","
                 << viz_ms;
  }
  ...
  timing_file_ << "," << max_cloud_ms << "," << timing_data->combined_publish_ms
               << "," << total_wall_ms;
...
void DepthEstimation::InitializeRosParameters() {
  ...
  // NOTE: depth_correction parameters are loaded from calibration folder in
  // InitializeCalibrationData()

  if (verbose_) {
    RCLCPP_INFO(
        this->get_logger(),
        "Verbose mode ENABLED: Publishing 2x4 Debug Grid (Left|Disparity).");
  }

  RCLCPP_INFO(this->get_logger(), "Execution Mode: %s",
              run_parallel_ ? "PARALLEL (Multi-stream)"
                            : "SEQUENTIAL (Single-stream)");
  RCLCPP_INFO(this->get_logger(), "Async Pointcloud Workers: %d",
              num_pointcloud_workers_);
  ...
  if (model_type_ == "s2m2" && filter_disparity_) {
    RCLCPP_INFO(this->get_logger(),
                "S2M2 Confidence & Occlusion filtering ENABLED.");
  }
}
...
void DepthEstimation::InitializeLogging() {
  ...
  try {
    ...
    if (!timing_file_.is_open()) {
      RCLCPP_ERROR(this->get_logger(), "Failed to open logging file: %s",
                   filename.c_str());
      return;
    }
    ...
    RCLCPP_INFO(this->get_logger(), "Logging timing data to: %s",
                filename.c_str());
...
void DepthEstimation::InitializeModel() {
  ...
  if (model_type_ == "hitnet") {
    if (input_node_dims_.size() != 1)
      throw std::runtime_error(
          "HitNet model must have exactly 1 input tensor.");
    model_full_height_ = input_node_dims_[0][2];
    model_full_width_ = input_node_dims_[0][3];
    RCLCPP_INFO(this->get_logger(), "Detected HitNet input resolution: %dx%d",
                model_full_width_, model_full_height_);

  } else if (model_type_ == "s2m2") {
    if (input_node_dims_.size() != 2)
      throw std::runtime_error("S2M2 model must have exactly 2 input tensors.");
    model_full_height_ = input_node_dims_[0][2];
    model_full_width_ = input_node_dims_[0][3];
    RCLCPP_INFO(this->get_logger(), "Detected S2M2 input resolution: %dx%d",
                model_full_width_, model_full_height_);

  } else if (model_type_ == "crestereo") {
    if (input_node_dims_.size() == 4) {
      RCLCPP_INFO(this->get_logger(),
                  "Detected 4-input Crestereo model (Iterative Refinement).");
      ...
      RCLCPP_INFO(this->get_logger(),
                  "Detected 2-input Crestereo model (Single-Pass).");
      model_full_height_ = input_node_dims_[0][2];
      model_full_width_ = input_node_dims_[0][3];
    } else {
      throw std::runtime_error(
          "Unsupported Crestereo model. Must have 2 or 4 input tensors.");
    }
  }
  ...
  RCLCPP_INFO(this->get_logger(),
              "Creating final inference options with '%s' provider...",
  ...
  if (execution_provider_ == "tensorrt") {
    ...
    std::string cache_dir_name =
        stem + "_" + gpu_name + "_" + arch + "_trt_engine";
    std::filesystem::path cache_path = model_dir / cache_dir_name;

    RCLCPP_INFO(this->get_logger(), "TRT Cache Target: %s",
    ...
    if (!std::filesystem::exists(cache_path)) {
      ...
      try {
        ...
        RCLCPP_ERROR(this->get_logger(),
                     "Failed to create cache dir: %s. Fallback to /tmp",
                     e.what());
    ...
    try {
      ...
      RCLCPP_INFO(this->get_logger(),
                  "Successfully appended TensorRT execution provider options.");
    } catch (const Ort::Exception &e) {
      RCLCPP_ERROR(this->get_logger(), "Error appending TensorRT: %s",
                   e.what());
  ...
  if (execution_provider_ == "cuda") {
    ...
    try {
      ...
      RCLCPP_INFO(this->get_logger(),
                  "Successfully appended CUDA execution provider.");
    } catch (const Ort::Exception &e) {
  ...
  RCLCPP_INFO(this->get_logger(), "Creating %d Inference Session(s)...",
              num_sessions_to_create);
  ...
  for (int i = 0; i < num_sessions_to_create; i++) {
    try {
      ...
      RCLCPP_FATAL(this->get_logger(), "Failed to create session %d: %s", i,
                   e.what());
      throw;
    }
  }

  RCLCPP_INFO(this->get_logger(), "Successfully loaded %zu ONNX sessions.",
              ort_sessions_.size());
}
...
void DepthEstimation::InitializeCalibrationData() {
  ...
  const std::string calibration_dir =
      calibration_base_path_ + "/final_maps_" + calibration_resolution_;

  // --- LOAD DEPTH CORRECTIONS FROM FILE ---
  std::string depth_corrections_path =
      calibration_dir + "/depth_corrections.yaml";
  std::vector<double> baseline_scales;
  std::vector<double> offset_factors;

  if (std::filesystem::exists(depth_corrections_path)) {
    RCLCPP_INFO(this->get_logger(), "Loading depth corrections from: %s",
    ...
    try {
      ...
      if (corrections["depth_correction"]) {
        if (corrections["depth_correction"]["baseline_scales"]) {
          baseline_scales = corrections["depth_correction"]["baseline_scales"]
                                .as<std::vector<double>>();
        }
        if (corrections["depth_correction"]["offset_factors"]) {
          offset_factors = corrections["depth_correction"]["offset_factors"]
                               .as<std::vector<double>>();
        }
      }
    } catch (const std::exception &e) {
      RCLCPP_WARN(this->get_logger(),
                  "Failed to parse depth_corrections.yaml: %s. Using defaults.",
                  e.what());
    }
  } else {
    RCLCPP_WARN(this->get_logger(),
                "depth_corrections.yaml not found at %s. Using default values "
                "(no correction).",
                depth_corrections_path.c_str());
  }
  ...
  if (baseline_scales.size() != stereo_pairs_.size()) {
    if (!baseline_scales.empty()) {
      RCLCPP_WARN(this->get_logger(),
                  "baseline_scales size (%zu) doesn't match stereo_pairs size "
                  "(%zu). Using default 1.0.",
  ...
  if (offset_factors.size() != stereo_pairs_.size()) {
    if (!offset_factors.empty()) {
      RCLCPP_WARN(this->get_logger(),
                  "offset_factors size (%zu) doesn't match stereo_pairs size "
                  "(%zu). Using default 0.0.",
  ...
  RCLCPP_INFO(this->get_logger(), "Formula: Z_corrected = baseline_scale * "
                                  "Z_raw / (1 + offset_factor * Z_raw)");
  for (size_t i = 0; i < stereo_pairs_.size(); ++i) {
    RCLCPP_INFO(this->get_logger(),
                "  Pair %s: baseline_scale=%.6f, offset_factor=%.6f",
                stereo_pairs_[i].c_str(), baseline_scales[i],
                offset_factors[i]);
  }
  ...
  for (size_t pair_idx = 0; pair_idx < stereo_pairs_.size(); ++pair_idx) {
    ...
    std::string map_path = calibration_dir +
                           "/final_rectified_to_fisheye_map_" + pair_str +
                           ".yml";
    std::string config_path =
        calibration_dir + "/final_map_config_" + pair_str + ".yaml";
    ...
    auto k_values =
        config["K_rect_left"].as<std::vector<std::vector<double>>>();
    data.K_rect_left =
        (cv::Mat_<double>(3, 3) << k_values[0][0], k_values[0][1],
         k_values[0][2], k_values[1][0], k_values[1][1], k_values[1][2],
         k_values[2][0], k_values[2][1], k_values[2][2]);

    auto T_values =
        config["T_rect_left_cam0"].as<std::vector<std::vector<double>>>();
    for (int i = 0; i < 4; ++i) {
      for (int j = 0; j < 4; ++j) {
        data.transform_rect_left_to_cam0(i, j) =
            static_cast<float>(T_values[i][j]);
      }
    }
    ...
    RCLCPP_INFO(
        this->get_logger(),
        "Loaded pair %s: baseline=%.6f m, scale=%.6f, offset_factor=%.6f",
        pair_str.c_str(), data.baseline_meters, data.baseline_scale,
        data.offset_factor);
...
void DepthEstimation::InitializePublishers() {
  ...
  for (const auto &pair_str : stereo_pairs_) {
    ...
    pointcloud_pubs_[pair_str] =
        this->create_publisher<sensor_msgs::msg::PointCloud2>(topic_name, 10);
  }

  if (debug_grid_) {
    debug_grid_pub_ = this->create_publisher<sensor_msgs::msg::CompressedImage>(
        "/depth/debug_grid/compressed", 10);
  }
...
void DepthEstimation::ProcessImage(
  ...
  timing_data->callback_start =
      t_start_total; // Store callback start time for total wall time
  ...
  if (publish_combined_pointcloud_) {
    ...
    size_t points_per_pair =
        first_pair_data.resolution.width * first_pair_data.resolution.height;

    // Create aggregator with pre-allocated PointCloud2 buffer (no PCL
    // intermediate)
  ...
  auto process_pair_task = [&](const std::string &pair_str, int s_idx,
    ...
    res.rect_time_ms =
        std::chrono::duration<double, std::milli>(
            std::chrono::high_resolution_clock::now() - t_start_rect)
            .count();

    // B. INFERENCE
    auto t_start_infer = std::chrono::high_resolution_clock::now();
    cv::Mat disparity_map =
        RunInference(rectified_left, rectified_right, s_idx);
    res.infer_time_ms =
        std::chrono::duration<double, std::milli>(
            std::chrono::high_resolution_clock::now() - t_start_infer)
            .count();
    ...
    payload.debug_grid = debug_grid_;
  ...
  for (const auto &pair_str : stereo_pairs_) {
    ...
    if (run_parallel_) {
      futures.push_back(std::async(std::launch::async, process_pair_task,
                                   pair_str, session_idx, pair_idx));
    } else {
  ...
  double inference_wall_ms =
      std::chrono::duration<double, std::milli>(t_end_inference - t_start_total)
          .count();
  ...
  // Note: Full timing report will be printed by async workers when all pairs
  // complete
}
...
cv::Mat DepthEstimation::RunInference(const cv::Mat &rectified_left,
  ...
  if (model_type_ == "s2m2") {
    cv::Mat blob_left = cv::dnn::blobFromImage(
        rectified_left, 1.0, cv::Size(model_full_width_, model_full_height_),
        cv::Scalar(), true, false, CV_8U);
    cv::Mat blob_right = cv::dnn::blobFromImage(
        rectified_right, 1.0, cv::Size(model_full_width_, model_full_height_),
        cv::Scalar(), true, false, CV_8U);

    input_tensors.push_back(Ort::Value::CreateTensor<uint8_t>(
        allocator.GetInfo(), blob_left.ptr<uint8_t>(), blob_left.total(),
        input_node_dims_[0].data(), input_node_dims_[0].size()));
    input_tensors.push_back(Ort::Value::CreateTensor<uint8_t>(
        allocator.GetInfo(), blob_right.ptr<uint8_t>(), blob_right.total(),
        input_node_dims_[1].data(), input_node_dims_[1].size()));
    ...
    cv::resize(left_gray, left_resized,
               cv::Size(model_full_width_, model_full_height_));
    cv::resize(right_gray, right_resized,
               cv::Size(model_full_width_, model_full_height_));
    ...
    cv::Mat blob =
        cv::dnn::blobFromImages(images, 1.0, cv::Size(), cv::Scalar(), false);
    std::vector<int64_t> input_shape = {1, 2, model_full_height_,
                                        model_full_width_};
    input_tensors.push_back(Ort::Value::CreateTensor<float>(
        allocator.GetInfo(), blob.ptr<float>(), blob.total(),
    ...
    if (input_node_names_.size() == 4) {
      cv::Mat left_full_resized, right_full_resized, left_half_resized,
          right_half_resized;
      cv::resize(left_bgr, left_full_resized,
                 cv::Size(model_full_width_, model_full_height_));
      cv::resize(right_bgr, right_full_resized,
                 cv::Size(model_full_width_, model_full_height_));
      cv::resize(left_bgr, left_half_resized,
                 cv::Size(model_half_width_, model_half_height_));
      cv::resize(right_bgr, right_half_resized,
                 cv::Size(model_half_width_, model_half_height_));
      ...
      input_tensors.push_back(Ort::Value::CreateTensor<float>(
          allocator.GetInfo(), blob_left_half.ptr<float>(),
          blob_left_half.total(), input_node_dims_[0].data(),
          input_node_dims_[0].size()));
      input_tensors.push_back(Ort::Value::CreateTensor<float>(
          allocator.GetInfo(), blob_right_half.ptr<float>(),
          blob_right_half.total(), input_node_dims_[1].data(),
          input_node_dims_[1].size()));
      input_tensors.push_back(Ort::Value::CreateTensor<float>(
          allocator.GetInfo(), blob_left.ptr<float>(), blob_left.total(),
          input_node_dims_[2].data(), input_node_dims_[2].size()));
      input_tensors.push_back(Ort::Value::CreateTensor<float>(
          allocator.GetInfo(), blob_right.ptr<float>(), blob_right.total(),
          input_node_dims_[3].data(), input_node_dims_[3].size()));

    } else if (input_node_names_.size() == 2) {
      cv::Mat left_resized, right_resized;
      cv::resize(left_bgr, left_resized,
                 cv::Size(model_full_width_, model_full_height_));
      cv::resize(right_bgr, right_resized,
                 cv::Size(model_full_width_, model_full_height_));
  ...
  auto output_tensors = ort_sessions_[session_idx]->Run(
      ...
      input_tensors.size(), output_names_char.data(), output_names_char.size());
  ...
  if (model_type_ == "hitnet") {
    ...
    cv::Mat disparity_small(model_full_height_, model_full_width_, CV_32F,
                            float_buffer);
    cv::resize(disparity_small, disparity_map,
               cv::Size(original_width, original_height), 0, 0,
               cv::INTER_LINEAR);
    disparity_map *= (static_cast<float>(original_width) /
                      static_cast<float>(model_full_width_));
    ...
    cv::resize(disp_small, disparity_map,
               cv::Size(rectified_left.cols, rectified_left.rows), 0, 0,
               cv::INTER_LINEAR);
    disparity_map *=
        (static_cast<float>(rectified_left.cols) / static_cast<float>(out_w));
    ...
    if (filter_disparity_ && output_tensors.size() >= 3) {
      ...
      cv::resize(occ_small, occ_map,
                 cv::Size(rectified_left.cols, rectified_left.rows), 0, 0,
                 cv::INTER_LINEAR);
      cv::resize(conf_small, conf_map,
                 cv::Size(rectified_left.cols, rectified_left.rows), 0, 0,
                 cv::INTER_LINEAR);
...
// - Depth correction: Z_corrected = baseline_scale * Z_raw / (1 + offset_factor
// * Z_raw)
// =============================================================================
pcl::PointCloud<pcl::PointXYZ>::Ptr DepthEstimation::DisparityToPointCloud(
    const cv::Mat &disparity_map, const cv::Mat &K_rect_left,
    double baseline_meters, double baseline_scale, double offset_factor,
  ...
  // Pre-extract transform components for faster access (avoid Eigen overhead in
  // loop)
  const float r00 = combined_transform(0, 0), r01 = combined_transform(0, 1),
              r02 = combined_transform(0, 2), t0 = combined_transform(0, 3);
  const float r10 = combined_transform(1, 0), r11 = combined_transform(1, 1),
              r12 = combined_transform(1, 2), t1 = combined_transform(1, 3);
  const float r20 = combined_transform(2, 0), r21 = combined_transform(2, 1),
              r22 = combined_transform(2, 2), t2 = combined_transform(2, 3);
  ...
  for (int v = 0; v < rows; ++v) {
    ...
    for (int u = 0; u < cols; ++u) {
      ...
      if (disparity > 0.5f) {
        ...
        // Step 4: Apply combined transform inline (rect_left -> cam0 ->
        // centroid)
...
void DepthEstimation::DisparityToPointCloud2Direct(
    const cv::Mat &disparity_map, const cv::Mat &K_rect_left,
    double baseline_meters, double baseline_scale, double offset_factor,
    const Eigen::Matrix4f &combined_transform, float *output_buffer) {
...
void DepthEstimation::InitializeServices() {
  camera_info_service_ =
      this->create_service<depth_estimation_ros2::srv::GetCameraInfo>(
          "~/get_camera_info",
          std::bind(&DepthEstimation::GetCameraInfoCallback, this,
...
void DepthEstimation::GetCameraInfoCallback(
  ...
  for (const auto &pair_str : stereo_pairs_) {
    ...
    if (processed_cameras.find(cam_idx) == processed_cameras.end()) {
      ...
      Eigen::Matrix4f transform_rect_cam_to_centroid =
          transform_cam0_to_centroid_ * pair_data.transform_rect_left_to_cam0;
